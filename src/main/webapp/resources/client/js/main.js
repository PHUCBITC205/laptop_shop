(function ($) {
    ...
    $(document).on('click', '.quantity button', function () {
        ...
        // Handle Guest Cart
        const guestId = input.attr("data-guest-id");
        if (guestId) {
            if (typeof GuestCart !== 'undefined') {
                GuestCart.update(guestId, newVal);
                location.reload();
                return;
            }
        }
        ...
        if (totalPriceElements.length) {
            const currentTotal = parseFloat(totalPriceElements.first().attr("data-cart-total-price"));
            const newTotal = currentTotal + (change * parseFloat(price || 0));
