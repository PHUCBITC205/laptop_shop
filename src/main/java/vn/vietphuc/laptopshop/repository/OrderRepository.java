...
public interface OrderRepository extends JpaRepository<Order, Long>, JpaSpecificationExecutor<Order> {
    Page<Order> findAllByDeletedFalse(Pageable page);
    ...
    @org.springframework.data.jpa.repository.Query("SELECT SUM(o.totalPrice) FROM Order o " +
           "WHERE o.deleted = false AND (UPPER(o.status) LIKE '%COMPLETE%' OR UPPER(o.status) LIKE '%SUCCESS%') " +
    ...
    @org.springframework.data.jpa.repository.Query("SELECT COUNT(o) FROM Order o WHERE o.deleted = false AND (UPPER(o.status) LIKE '%COMPLETE%' OR UPPER(o.status) LIKE '%SUCCESS%')")
    long countCompleteOrders();

    @org.springframework.data.jpa.repository.Query("SELECT o.status, COUNT(o) FROM Order o WHERE o.deleted = false GROUP BY o.status")
